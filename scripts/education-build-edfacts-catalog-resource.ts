...
const root = 'data/education', prefix = 'edfacts-catalog-resource';
...
const name = 'EDFacts / Ed Data Express';
const description = 'EDFacts provides annual school-, district-, and state-level education data with subgroup detail. It supplies outcome and accountability context; it does not establish an initiative effect.';
const officialUrl = 'https://eddataexpress.ed.gov/download/data-builder/data-download-tool';
const content = `## Catalog metadata\n\n- **Steward:** U.S. Department of Education.\n- **Access:** Custom CSV downloads and released ZIP files with data notes.\n- **Unit:** Education agency or school, annual and subgroup-disaggregated.\n- **Coverage:** Varies by file specification and outcome; record school year and data group.\n- **Join keys:** State, LEA ID, school ID, school year, and subgroup.\n- **Measures:** Assessment, graduation, chronic absence, accountability, CTE, and English-learner progress.\n- **Use boundary:** Preserve file specification, data notes, suppression rules, and denominator with every extract.\n\nEDFacts can provide attendance, graduation, subgroup, implementation, and accountability context. Its released values do not alone establish that a program caused an outcome.`;
const checks: string[] = [];
...
for (const term of ['EDFacts', 'Ed Data Express', 'Education Data Express']) {
...
const ops: Op[] = [...Ops.entities.update({ id: dataset, name, description, values: [{ property: p.url, type: 'text', value: officialUrl }] }).ops, ...Ops.entities.update({ id: block, name: 'EDFacts catalog metadata', values: [{ property: p.markdown, type: 'text', value: content }] }).ops];
...
const batch = { name: 'Add EDFacts catalog resource and metadata block', spaceId: target.spaceId, bounty: target.bountyId, opsPath: `${root}/${prefix}-ops.json`, sha256, journalPath: `${root}/${prefix}-publication.json`, validationPath: `${root}/${prefix}-validation.json`, entities: { dataset, block }, operationCount: ops.length };
writeFileSync(registryPath, JSON.stringify(registry, null, 2) + '\n'); writeFileSync(batch.opsPath, bytes); writeFileSync(`${root}/${prefix}-batch.json`, JSON.stringify(batch, null, 2) + '\n');
writeFileSync(batch.validationPath, JSON.stringify({ ready: true, checkedAt: new Date().toISOString(), opsHash: sha256, checks, source: { catalogKey: 'edfacts-ed-data-express', sourcePath: 'Education-Initiatives/research-data/dataset-catalog.json', officialUrl }, scope: 'Catalog metadata only. No raw EDFacts data, derived outcome, causal claim, initiative effect, or unsupported initiative relation is published.' }, null, 2) + '\n');
