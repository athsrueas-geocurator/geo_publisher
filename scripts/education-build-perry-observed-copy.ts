...
const root='data/education',prefix='perry-observed-copy',read=(n:string)=>JSON.parse(readFileSync(`${root}/${n}.json`,'utf8'));
...
for(const row of input.records.filter((r:any)=>r.kind==='observed-descriptive-group-mean')){
 const id=registry[`observed/${row.key}`],before=audit.rows.find((r:any)=>r.id===id);
 ...
 check(Number(v(prop(row.unit==='fraction'?'observedProportion':'observedMonetaryMean'))?.decimal)===Number(row.value)&&Number(v(prop('standardError'))?.decimal)===Number(row.standardError),`Exact source mean and SE ${row.key}`);
 check(row.causalEffect===false&&row.sampleSize===null,'No inferred treatment effect or outcome N');
 const after=perryObservedCopy(row);check(after.description.length<=350,`Concise description ${row.key}`);
...
check(review.length===32,'All 32 observed means covered');
...
const batch={name:'State Perry Preschool observed outcomes as factual claims',spaceId:target.spaceId,bounty:target.bountyId,opsPath:`${root}/${prefix}-ops.json`,sha256,journalPath:`${root}/${prefix}-publication.json`,validationPath:`${root}/${prefix}-validation.json`};
