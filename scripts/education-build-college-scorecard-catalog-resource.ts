...
const root = 'data/education', prefix = 'college-scorecard-catalog-resource';
...
const name = 'College Scorecard';
const description = 'College Scorecard provides institution- and cohort-level postsecondary completion, earnings, debt, repayment, and aid data. It supplies pathway and return context; it does not establish that an initiative caused an outcome.';
const officialUrl = 'https://collegescorecard.ed.gov/data/';
const content = `## Catalog metadata\n\n- **Steward:** U.S. Department of Education.\n- **Access:** Downloadable files and API.\n- **Unit:** Postsecondary institution and student-cohort summaries.\n- **Coverage:** Release-dependent historical files.\n- **Geography:** Institution, state, and nation.\n- **Join keys:** UNITID, OPEID, state, and reporting year.\n- **Measures:** Completion, earnings, debt, repayment, and aid.\n- **Use boundary:** Preserve released cohort definitions; institution averages are not individual program effects.\n\nCollege Scorecard provides career-pathway and postsecondary-return context. Its released values do not alone establish that a program caused an outcome.`;
const checks: string[] = [];
...
for (const term of ['CollegeScorecard']) {
...
const ops: Op[] = [...Ops.entities.update({ id: dataset, name, description, values: [{ property: p.url, type: 'text', value: officialUrl }] }).ops, ...Ops.entities.update({ id: block, name: 'College Scorecard catalog metadata', values: [{ property: p.markdown, type: 'text', value: content }] }).ops];
...
const batch = { name: 'Add College Scorecard catalog resource and metadata block', spaceId: target.spaceId, bounty: target.bountyId, opsPath: `${root}/${prefix}-ops.json`, sha256, journalPath: `${root}/${prefix}-publication.json`, validationPath: `${root}/${prefix}-validation.json`, entities: { dataset, block }, operationCount: ops.length };
writeFileSync(registryPath, JSON.stringify(registry, null, 2) + '\n'); writeFileSync(batch.opsPath, bytes); writeFileSync(`${root}/${prefix}-batch.json`, JSON.stringify(batch, null, 2) + '\n');
writeFileSync(batch.validationPath, JSON.stringify({ ready: true, checkedAt: new Date().toISOString(), opsHash: sha256, checks, source: { catalogKey: 'college-scorecard', sourcePath: 'Education-Initiatives/research-data/dataset-catalog.json', officialUrl }, scope: 'Catalog metadata only. No raw College Scorecard data, derived outcome, causal claim, initiative effect, or unsupported initiative relation is published.' }, null, 2) + '\n');
