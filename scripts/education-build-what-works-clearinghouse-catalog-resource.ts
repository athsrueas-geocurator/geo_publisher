...
const root = 'data/education', prefix = 'what-works-clearinghouse-catalog-resource';
...
const name = 'What Works Clearinghouse';
const description = 'What Works Clearinghouse follows a nationally representative kindergarten cohort from 2010 through fifth grade. It supports weighted descriptive and developmental context; it does not by itself estimate an initiative’s effect.';
const officialUrl = 'https://ies.ed.gov/ncee/wwc/';
...
- **Unit:** Reviewed intervention or study.
- **Coverage:** Continuously updated repository.
- **Geography:** Study-dependent.
- **Join keys:** Student record, wave, permitted school record, and survey weight.
- **Measures:** Study quality, outcome domain, effect direction, and evidence tier.
- **Appropriate uses:** Study-quality normalization and evidence discovery.

Use NCES weights and cohort-specific documentation. Public-use identifiers are suppressed, and What Works Clearinghouse is observational longitudinal data, so causal initiative evaluation requires a separate credible design.`;
const checks: string[] = [];
const check = (value: unknown, message: string) => { if (!value) throw new Error(message); checks.push(message); };
const discovery = JSON.parse(readFileSync(`${root}/discovery/what-works-clearinghouse.json`, 'utf8'));
check(discovery.term === 'What Works Clearinghouse', 'Discovery report belongs to What Works Clearinghouse');
...
const ops: Op[] = [
  ...
  ...Ops.entities.update({ id: block, name: 'What Works Clearinghouse catalog metadata', values: [{ property: p.markdown, type: 'text', value: content }] }).ops,
];
...
const batch = { name: 'Add What Works Clearinghouse catalog resource and metadata block', spaceId: target.spaceId, bounty: target.bountyId, opsPath: `${root}/${prefix}-ops.json`, sha256, journalPath: `${root}/${prefix}-publication.json`, validationPath: `${root}/${prefix}-validation.json`, entities: { dataset, block }, operationCount: ops.length };
writeFileSync(registryPath, JSON.stringify(registry, null, 2) + '\n');
writeFileSync(batch.opsPath, bytes);
writeFileSync(`${root}/${prefix}-batch.json`, JSON.stringify(batch, null, 2) + '\n');
writeFileSync(batch.validationPath, JSON.stringify({ ready: true, checkedAt: new Date().toISOString(), opsHash: sha256, checks, source: { catalogKey: 'hsls-09', sourcePath: 'data/education/source/research-data/dataset-catalog.json', officialUrl, accessUrl: 'https://ies.ed.gov/ncee/wwc/dataproducts.asp' }, scope: 'Catalog metadata only. No What Works Clearinghouse microdata, derived outcome, causal claim, initiative effect, or unsupported initiative relation is published.' }, null, 2) + '\n');
