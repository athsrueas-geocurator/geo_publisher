...
const root = 'data/education', prefix = 'naep-catalog-resource';
...
const name = 'NAEP Data Explorer';
const description = 'NAEP Data Explorer provides representative assessment results across national, state, and participating urban-district samples. It supplies benchmark and trend context; it does not establish that an initiative caused an outcome.';
const officialUrl = 'https://nces.ed.gov/nationsreportcard/naepdata/';
const content = `## Catalog metadata\n\n- **Steward:** National Center for Education Statistics.\n- **Access:** Custom tables and charts; qualified users may access restricted-use microdata.\n- **Unit:** Representative assessment result and subgroup.\n- **Coverage:** Assessment-specific trend series across decades.\n- **Geography:** Nation, state, and participating urban district; not school level.\n- **Join keys:** Jurisdiction, assessment year, grade, subject, and subgroup.\n- **Measures:** Scale scores, achievement levels, and questionnaire context.\n- **Use boundary:** Preserve the assessment, population, and sampling context.\n\nNAEP supports national/state benchmarks and achievement-trend context. It must not be used alone to attribute an initiative effect.`;
const checks: string[] = [];
...
for (const term of ['National Assessment of Educational Progress Data Explorer']) {
...
const ops: Op[] = [...Ops.entities.update({ id: dataset, name, description, values: [{ property: p.url, type: 'text', value: officialUrl }] }).ops, ...Ops.entities.update({ id: block, name: 'NAEP catalog metadata', values: [{ property: p.markdown, type: 'text', value: content }] }).ops];
...
const batch = { name: 'Add NAEP catalog resource and metadata block', spaceId: target.spaceId, bounty: target.bountyId, opsPath: `${root}/${prefix}-ops.json`, sha256, journalPath: `${root}/${prefix}-publication.json`, validationPath: `${root}/${prefix}-validation.json`, entities: { dataset, block }, operationCount: ops.length };
writeFileSync(registryPath, JSON.stringify(registry, null, 2) + '\n'); writeFileSync(batch.opsPath, bytes); writeFileSync(`${root}/${prefix}-batch.json`, JSON.stringify(batch, null, 2) + '\n');
writeFileSync(batch.validationPath, JSON.stringify({ ready: true, checkedAt: new Date().toISOString(), opsHash: sha256, checks, source: { catalogKey: 'naep-data-explorer', sourcePath: 'Education-Initiatives/research-data/dataset-catalog.json', officialUrl }, scope: 'Catalog metadata only. No raw NAEP data, derived outcome, causal claim, initiative effect, or unsupported initiative relation is published.' }, null, 2) + '\n');
