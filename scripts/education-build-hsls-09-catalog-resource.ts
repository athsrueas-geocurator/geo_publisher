...
const root = 'data/education', prefix = 'hsls-09-catalog-resource';
...
const name = 'High School Longitudinal Study of 2009 (HSLS:09)';
const description = 'HSLS:09 follows a nationally representative kindergarten cohort from 2010 through fifth grade. It supports weighted descriptive and developmental context; it does not by itself estimate an initiative’s effect.';
const officialUrl = 'https://nces.ed.gov/surveys/hsls09/';
...
- **Unit:** Longitudinal student record with linked parent, teacher, counselor, school, transcript, and postsecondary records.
- **Coverage:** 2009 ninth-grade cohort through 2021 postsecondary administrative records.
- **Geography:** Nation and selected states.
- **Join keys:** Student record, wave, permitted school record, and survey weight.
- **Measures:** Course taking, CTE participation, math assessment, college expectations, and postsecondary enrollment and completion.
- **Appropriate uses:** Postsecondary pathways, course-taking, CTE, and college-readiness context.

Use NCES weights and cohort-specific documentation. Public-use identifiers are suppressed, and HSLS:09 is observational longitudinal data, so causal initiative evaluation requires a separate credible design.`;
const checks: string[] = [];
const check = (value: unknown, message: string) => { if (!value) throw new Error(message); checks.push(message); };
const discovery = JSON.parse(readFileSync(`${root}/discovery/high-school-longitudinal-study-of-2009-hsls-09.json`, 'utf8'));
check(discovery.term === 'High School Longitudinal Study of 2009 (HSLS:09)', 'Discovery report belongs to HSLS:09');
...
const ops: Op[] = [
  ...
  ...Ops.entities.update({ id: block, name: 'HSLS:09 catalog metadata', values: [{ property: p.markdown, type: 'text', value: content }] }).ops,
];
...
const batch = { name: 'Add HSLS:09 catalog resource and metadata block', spaceId: target.spaceId, bounty: target.bountyId, opsPath: `${root}/${prefix}-ops.json`, sha256, journalPath: `${root}/${prefix}-publication.json`, validationPath: `${root}/${prefix}-validation.json`, entities: { dataset, block }, operationCount: ops.length };
writeFileSync(registryPath, JSON.stringify(registry, null, 2) + '\n');
writeFileSync(batch.opsPath, bytes);
writeFileSync(`${root}/${prefix}-batch.json`, JSON.stringify(batch, null, 2) + '\n');
writeFileSync(batch.validationPath, JSON.stringify({ ready: true, checkedAt: new Date().toISOString(), opsHash: sha256, checks, source: { catalogKey: 'hsls-09', sourcePath: 'data/education/source/research-data/dataset-catalog.json', officialUrl, accessUrl: 'https://nces.ed.gov/surveys/hsls09/dataproducts.asp' }, scope: 'Catalog metadata only. No HSLS:09 microdata, derived outcome, causal claim, initiative effect, or unsupported initiative relation is published.' }, null, 2) + '\n');
