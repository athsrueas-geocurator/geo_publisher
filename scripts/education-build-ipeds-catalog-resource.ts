...
const root = 'data/education', prefix = 'ipeds-catalog-resource';
...
const name = 'Integrated Postsecondary Education Data System (IPEDS)';
const description = 'IPEDS provides annual institution- and cohort-level postsecondary education data. It supplies pathway and completion context; it does not establish that an initiative caused an outcome.';
const officialUrl = 'https://nces.ed.gov/ipeds/';
const content = `## Catalog metadata\n\n- **Steward:** National Center for Education Statistics.\n- **Access:** Downloadable complete data files and custom data tools.\n- **Unit:** Postsecondary institution and cohort.\n- **Coverage:** Annual releases with component-specific history.\n- **Geography:** Institution, state, and nation.\n- **Join keys:** UNITID, state, and reporting year.\n- **Measures:** Enrollment, awards, graduation, finance, and staffing.\n- **Use boundary:** Institutional outcomes are not direct student-level follow-up for a K–12 initiative.\n\nIPEDS provides postsecondary pathway supply and completion context. Its released values do not alone establish that a program caused an outcome.`;
const checks: string[] = [];
...
for (const term of ['Integrated Postsecondary Education Data System']) {
...
const ops: Op[] = [...Ops.entities.update({ id: dataset, name, description, values: [{ property: p.url, type: 'text', value: officialUrl }] }).ops, ...Ops.entities.update({ id: block, name: 'IPEDS catalog metadata', values: [{ property: p.markdown, type: 'text', value: content }] }).ops];
...
const batch = { name: 'Add IPEDS catalog resource and metadata block', spaceId: target.spaceId, bounty: target.bountyId, opsPath: `${root}/${prefix}-ops.json`, sha256, journalPath: `${root}/${prefix}-publication.json`, validationPath: `${root}/${prefix}-validation.json`, entities: { dataset, block }, operationCount: ops.length };
writeFileSync(registryPath, JSON.stringify(registry, null, 2) + '\n'); writeFileSync(batch.opsPath, bytes); writeFileSync(`${root}/${prefix}-batch.json`, JSON.stringify(batch, null, 2) + '\n');
writeFileSync(batch.validationPath, JSON.stringify({ ready: true, checkedAt: new Date().toISOString(), opsHash: sha256, checks, source: { catalogKey: 'ipeds', sourcePath: 'Education-Initiatives/research-data/dataset-catalog.json', officialUrl }, scope: 'Catalog metadata only. No raw IPEDS data, derived outcome, causal claim, initiative effect, or unsupported initiative relation is published.' }, null, 2) + '\n');
