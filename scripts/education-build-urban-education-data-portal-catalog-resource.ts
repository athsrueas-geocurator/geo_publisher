...
const root = 'data/education', prefix = 'urban-education-data-portal-catalog-resource';
...
const name = 'Urban Institute Education Data Portal';
const description = 'Urban Institute Education Data Portal provides programmatic access to selected public education data. It supplies delivery and field-shape context; it does not establish that an initiative caused an outcome.';
const officialUrl = 'https://educationdata.urban.org/documentation/index.html';
const content = `## Catalog metadata\n\n- **Steward:** Urban Institute.\n- **Access:** Public JSON API.\n- **Unit:** Source-dependent public administrative data.\n- **Coverage:** Endpoint-dependent and source-dependent.\n- **Geography:** School, district, college, and state.\n- **Join keys:** NCES school ID, LEA ID, UNITID, FIPS, and year.\n- **Uses:** Programmatic extraction, rapid field-shape validation, and prototype queries.\n- **Provenance:** Use the originating agency as the authority for published claims.\n\nThe portal can deliver CCD, CRDC, EDFacts, IPEDS, and related public data. Delivery access does not turn a portal extract into causal evidence.`;
const checks: string[] = [];
...
for (const term of ['Education Data Portal']) {
...
const ops: Op[] = [...Ops.entities.update({ id: dataset, name, description, values: [{ property: p.url, type: 'text', value: officialUrl }] }).ops, ...Ops.entities.update({ id: block, name: 'Urban Education Data Portal catalog metadata', values: [{ property: p.markdown, type: 'text', value: content }] }).ops];
...
const batch = { name: 'Add Urban Education Data Portal catalog resource and metadata block', spaceId: target.spaceId, bounty: target.bountyId, opsPath: `${root}/${prefix}-ops.json`, sha256, journalPath: `${root}/${prefix}-publication.json`, validationPath: `${root}/${prefix}-validation.json`, entities: { dataset, block }, operationCount: ops.length };
writeFileSync(registryPath, JSON.stringify(registry, null, 2) + '\n'); writeFileSync(batch.opsPath, bytes); writeFileSync(`${root}/${prefix}-batch.json`, JSON.stringify(batch, null, 2) + '\n');
writeFileSync(batch.validationPath, JSON.stringify({ ready: true, checkedAt: new Date().toISOString(), opsHash: sha256, checks, source: { catalogKey: 'urban-education-data-portal', sourcePath: 'Education-Initiatives/research-data/dataset-catalog.json', officialUrl }, scope: 'Catalog metadata only. No raw portal data, derived outcome, causal claim, initiative effect, or unsupported initiative relation is published.' }, null, 2) + '\n');
